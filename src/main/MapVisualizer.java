    ...
    private String drawObject(Vector2D currentPosition) {
        ...
        if (this.map.isOccupied(currentPosition)) {
            ...
            if (object instanceof Grass) {
                ...
                Animal[] animals= (Animal[]) ((Collection) object).toArray(new Animal[((Collection) object).size()]);
                Integer n=animals.length;
                if(n==1) result=animals[0].toString();
                else result=n.toString().concat(" ");
            }
            else {
                result = EMPTY_CELL;
            }
